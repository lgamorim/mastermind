...
- Never write production code without a failing test that motivates it. Never mark a task complete while any test fails. Exception: when porting or adapting already-designed code (e.g., a pre-written draft) rather than discovering new behavior, there is no red step — treat `dotnet test` passing as the gate.
